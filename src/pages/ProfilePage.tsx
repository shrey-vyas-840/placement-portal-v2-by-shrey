...
export function ProfilePage() {
  ...
  const isInstitutionalEmail =
    !!user?.email && /^[A-Za-z0-9._%+-]+@([A-Za-z0-9-]+\.)?indusuni\.ac\.in$/i.test(user.email);
  ...
  useEffect(() => {
    ...
    studentService
      ...
      .then((data) => {
        ...
        if (data?.student_id) {
          loadAcademicDetails(data.student_id);
        }
        ...
        if (data?.student_id) {
          documentService
            ...
            .then((resume: any) => {
              const url = resume?.document_metadata?.storage_url ?? null;
  ...
  const loadResume = async (studentId: string) => {
    try {
      const resume: any = await documentService.getResume(studentId);

      console.log("LATEST RESUME", resume);

      const metadata = Array.isArray(resume?.document_metadata)
        ? resume.document_metadata
            .filter((x: any) => x?.storage_url)
            .sort(
              (a: any, b: any) =>
      ...
      console.log("RESUME RESPONSE", JSON.stringify(resume, null, 2));

      console.log("RESUME URL", metadata?.storage_url);
  ...
  const loadAcademicDetails = async (studentId: string) => {
    try {
      const data = await academicService.getAcademicDetails(studentId);
  ...
  const handleProfileSave = async () => {
    ...
    if (profile.alternate_contact_number && !/^[0-9]{10}$/.test(profile.alternate_contact_number)) {
      setError("Alternate contact number must contain exactly 10 digits.");
      return;
    }
    if (
    ...
    if (profile.middle_name && !/^[A-Za-z]$/.test(profile.middle_name.trim())) {
    ...
    try {
      ...
      const updated = await studentService.updateProfile(profile.student_id, {
        ...
        placement_preference: profile.placement_preference,
      });
      ...
      if (message.includes("chk_contact_number")) {
        setError("Contact number must contain exactly 10 digits.");
      } else {
...
function CompleteProfileForm({
  ...
  const [placementPreference, setPlacementPreference] = useState("Interested");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!/^IU[0-9]{8,13}$/.test(normalizedEnrollment)) {
      setError("Enrollment number must start with IU and contain 8 to 13 digits after it.");
      return;
    }
    ...
    if (personalEmail) {
      const emailValue = personalEmail.trim().toLowerCase();

      if (!/^[a-z0-9]+([._%+-]?[a-z0-9]+)*@[a-z0-9-]+\.(com|in|org|edu|net)$/i.test(emailValue)) {
        setError("Please enter a valid personal email address.");
        return;
      }
    }
    ...
    try {
      ...
      if (!account) {
        setError("User account was not provisioned correctly. Please logout and login again.");

        return;
      }

      const { error: insertError } = await (supabase as any).from("student_master").insert({
      ...
      console.error("PROFILE CREATE ERROR", err, JSON.stringify(err, null, 2));

      const message = err?.message ?? "";

      if (message.includes("chk_enrollment_pattern")) {
        setError(
          "Enrollment number must start with IU followed by numbers only. Example: IU2341230377",
        );
      } else if (message.includes("chk_contact_number")) {
        setError("Contact number must contain exactly 10 digits.");
      } else if (message.includes("student_master_enrollment_no_key")) {
  ...
  "
          ...
          onChange={(e) => setAlternateContactNumber(e.target.value.replace(/\D/g, ""))}
          required
        />
