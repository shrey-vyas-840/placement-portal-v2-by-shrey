  ...
  return (
    <div className="mx-auto max-w-7xl p-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Approval deadline:{" "}
            {formatDateTime(request?.hod_approval_deadline ?? context?.expiresAt)}
          </p>
        </div>
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <DetailCard
          label="HR Name"
          value={`${request?.snapshot?.hr_prefix ?? ""} ${request?.snapshot?.hr_name ?? "-"}`.trim()}
        />
        ...
        <DetailCard
          ...
          value={formatDateTime(request?.hod_approval_deadline)}
        />
        <DetailCard
          label="Token Status"
          value={context?.tokenRow?.used_at ? "Used" : context?.isExpired ? "Expired" : "Active"}
        />
      </div>
      ...
      <div className="mb-6 rounded-lg border bg-slate-50 p-4">
        ...
        <div className="mt-2 text-sm leading-6">
          {request?.snapshot?.student_name} is requesting NOC for {request?.noc_type} at{" "}
