...
function DonutChart({
  ...
  const legend = hideLegend ? null : (
    <div className={`grid gap-2 text-sm ${legendPosition === "right" ? "lg:max-w-[260px]" : ""}`}>
      {chartData.map((item) => (
        <div
          key={item.label}
  ...
  const centerBlock = (
    <div className="text-center">
      {centerTitle ? <div className="text-sm text-muted-foreground">{centerTitle}</div> : null}
...
export function AdminDashboardPage() {
  ...
  const branchDistribution: DriveBranchDistributionPoint[] =
    selectedDriveAnalyticsReady && snapshot?.branchDistribution ? snapshot.branchDistribution : [];

  const branchAnalytics: BranchAnalyticsItem[] =
    selectedDriveAnalyticsReady && snapshot?.branchAnalytics ? snapshot.branchAnalytics : [];

  const visibleBranches = showAllBranches ? branchAnalytics : branchAnalytics.slice(0, 2);

  useEffect(() => {
    setShowAllBranches(false);
  }, [selectedDriveId]);

  const pipeline: OpportunityPipelineReport | null = selectedDriveAnalyticsReady
    ? (snapshot?.pipeline ?? null)
    : null;

  const studentReport: StudentDrilldownReport | null = snapshot?.studentDrilldown ?? null;

  const filteredActivity = (snapshot?.recentActivity ?? []).filter((item) =>
  ...
  const studentPieData = useMemo(() => {
    ...
    if (pipeline) {
      ...
      const unregistered =
        pipeline.eligible_students != null
