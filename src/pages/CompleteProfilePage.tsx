...
export function CompleteProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!/^IU[0-9]+$/.test(normalizedEnrollment)) {
      setError("Enrollment number format is invalid. Example: IU2341230377");
      return;
    }
    ...
    try {
      ...
      if (message.includes("chk_contact_number")) {
        setError("Please enter a valid 10-digit mobile number.");
      } else if (message.includes("chk_enrollment_pattern")) {
        setError("Enrollment number format is invalid. Example: IU2341230377");
      } else if (message.includes("uq_student_enrollment")) {
  ...
  return (
    <div className="mx-auto max-w-xl p-8">
      ...
      {error && <div className="mb-4 rounded border border-red-300 p-3 text-red-600">{error}</div>}
