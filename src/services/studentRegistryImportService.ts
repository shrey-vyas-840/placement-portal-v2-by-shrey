...
export async function validateRegistryWorkbook(
    ...
    report.tests.push({
        label: "Header order",
        passed: headersMatch,
        note: headersMatch ? "Headers match the expected sequence." : "Header names or order do not match the template.",
    });
    ...
    if (!headersMatch) {
        addIssue(
            report.issues,
            "error",
            "Excel headers do not match the expected template exactly.",
        );
        report.errorCount = 1;
    ...
    for (let index = 0; index < rawRows.length; index += 1) {
        ...
        if (!timestamp) addIssue(issues, "error", "Timestamp is missing or invalid.", rowNumber, "Timestamp");
        if (!emailAddress) addIssue(issues, "error", "Email Address is missing or invalid.", rowNumber, "Email Address");
        if (!isInstitutionalEmail(emailAddress)) addIssue(issues, "error", "Email Address must be an indusuni.ac.in email.", rowNumber, "Email Address");
        if (!enrollmentNo) addIssue(issues, "error", "Enrollment No. is missing.", rowNumber, "Enrollment No.");
        if (!/^IU[0-9]+$/.test(enrollmentNo)) addIssue(issues, "error", "Enrollment No. must follow IU + digits.", rowNumber, "Enrollment No.");
        ...
        if (!gender) addIssue(issues, "error", "Gender must be Male, Female or Other.", rowNumber, "Gender");
        if (!dob) addIssue(issues, "error", "Date of Birth is missing or invalid.", rowNumber, "Date of Birth");
        if (dob && dob.getTime() > Date.now()) addIssue(issues, "error", "Date of Birth cannot be in the future.", rowNumber, "Date of Birth");
        if (!instituteEmailId) addIssue(issues, "error", "Institute Email ID is missing or invalid.", rowNumber, "Institute Email ID");
        if (!isInstitutionalEmail(instituteEmailId)) addIssue(issues, "error", "Institute Email ID must be an indusuni.ac.in email.", rowNumber, "Institute Email ID");
        if (emailAddress && instituteEmailId && emailAddress !== instituteEmailId) {
            addIssue(issues, "error", "Email Address and Institute Email ID must match.", rowNumber, "Email Address");
        }

        if (personalEmailId && !/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/.test(personalEmailId)) {
            addIssue(issues, "error", "Personal Email ID is invalid.", rowNumber, "Personal Email ID");
        }

        if (!contactNumber || contactNumber.length !== 10) {
            addIssue(issues, "error", "Contact No. must contain exactly 10 digits.", rowNumber, "Contact No.");
        }

        if (!hometown) addIssue(issues, "error", "Hometown is missing.", rowNumber, "Hometown");
        if (!currentDegree) addIssue(issues, "error", "Current Degree must be Master or Bachelor.", rowNumber, "Current Degree");

        if (currentDegree === "Master" && !mastersDegreeBranch) {
            addIssue(issues, "error", "Master's Degree - Branch is required for Master students.", rowNumber, "Master's Degree - Branch");
        }

        if (currentDegree === "Bachelor" && !bachelorsDegreeBranch) {
            addIssue(issues, "error", "Bachelor's Degree - Branch is required for Bachelor students.", rowNumber, "Bachelor's Degree - Branch");
        }

        if (!placementPreference) {
            addIssue(issues, "error", "Select Your Preference must be Opt - IN or Opt - OUT.", rowNumber, "Select Your Preference");
        }

        if (placementPreference === "Opt - IN") {
            if (sscPercentage === null) {
                addIssue(issues, "error", "SSC (10th) % is required for Opt - IN rows.", rowNumber, "SSC (10th) %");
            }

            if (!hscOrDiplomaType) {
                addIssue(issues, "error", "HSC (12th) or Diploma type is required for Opt - IN rows.", rowNumber, "HSC (12th) or Diploma?");
            }

            if (hscOrDiplomaPercentage === null) {
                addIssue(issues, "error", "% Marks of HSC (12th) or Diploma is required for Opt - IN rows.", rowNumber, "% Marks of HSC (12th) or Diploma");
            }

            if (!willingToRelocate) {
                addIssue(issues, "warning", "Willing to Relocate is empty for an Opt - IN row.", rowNumber, "Willing to Relocate for placement?");
            }

            if (reasonForOptOut) {
                addIssue(issues, "warning", "Reason for Opting out should normally be blank for Opt - IN rows.", rowNumber, "Reason for Opting out?");
            }
        }

        if (placementPreference === "Opt - OUT") {
            if (!reasonForOptOut) {
                addIssue(issues, "error", "Reason for Opting out is required for Opt - OUT rows.", rowNumber, "Reason for Opting out?");
            }
        }

        if (!declarationText) {
            addIssue(issues, "error", "Declaration text is missing.", rowNumber, "I hereby declare that .....");
        } else {
            const declarationOk =
                /placement policy/i.test(declarationText) ||
                /declare/i.test(declarationText);

            if (!declarationOk) {
                addIssue(issues, "warning", "Declaration text does not look complete.", rowNumber, "I hereby declare that .....");
            }
        }
    ...
    const existingEmailMap = new Map(existingRows.map((row) => [row.institute_email_id.toLowerCase(), row]));
        ...
        if (existingEnrollment) {
            ...
            if (existingTs > incomingTs) {
                ...
                row.issues.push({
                    ...
                    rowNumber: row.rowNumber,
                    field: "Enrollment No.",
                    message: "Database already has a newer record for this enrollment. This row will be skipped.",
                });
            } else {
                row.issues.push({
                    severity: "warning",
    ...
    report.warningCount = report.rows.reduce((sum, row) => sum + row.issues.filter((i) => i.severity === "warning").length, 0);
    report.errorCount = report.rows.reduce((sum, row) => sum + row.issues.filter((i) => i.severity === "error").length, 0);

    report.tests.push(
        {
            label: "Enrollment / email / contact validation",
            passed: report.errorCount === 0,
            note: report.errorCount === 0 ? "All hard validations passed." : "Some rows still have blocking issues.",
        },
        {
            label: "Duplicate resolution",
            passed: duplicateRowsMerged >= 0,
            note: duplicateRowsMerged > 0 ? `${duplicateRowsMerged} duplicate row(s) were merged by latest timestamp.` : "No duplicates inside the file.",
        },
        {
