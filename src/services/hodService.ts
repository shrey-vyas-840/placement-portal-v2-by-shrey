...
export const hodService = {
    ...
    async getPendingRequests(email: string) {
        const rows = await this.getRequestsByEmailAndStatus(email, [
            "PENDING_HOD_APPROVAL",
        ]);

        const tokens = await this.getTokenRowsForRequestIds(
            rows.map((row: any) => row.noc_request_id)
        );
    ...
    async createOrRefreshApprovalToken(
        nocRequestId: string,
        expiresAt: string
    ) {
