...
import type { StudentMasterRegistryRow }
...
export async function saveDraft(
    ...
    const nextRow = {
        ...
        onboarding_stage:
            input.onboardingStage ?? existing?.onboarding_stage ?? "OTP_VERIFIED",
        enrollment_no:
            input.enrollmentNo !== undefined
                ? input.enrollmentNo
                : existing?.enrollment_no ?? null,
        password_created:
            input.passwordCreated !== undefined
                ? input.passwordCreated
                : existing?.password_created ?? false,
        registry_found:
            input.registryFound !== undefined
                ? input.registryFound
                : existing?.registry_found ?? false,
        registry_snapshot:
            input.registrySnapshot !== undefined
                ? input.registrySnapshot
                : existing?.registry_snapshot ?? null,
        edited_profile:
            input.editedProfile !== undefined
                ? input.editedProfile
                : existing?.edited_profile ?? null,
