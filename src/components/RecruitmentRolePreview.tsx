...
function DocumentBadge({
  ...
  document: RecruitmentRoleDocument;
}) {
  return (
...
export function RecruitmentRolePreview({
  ...
  const requiredDocuments = role.documents.filter(
    (document) => document.required,
  );

  const optionalDocuments = role.documents.filter(
    (document) => !document.required,
  );

const requiredQuestions = role.questions.filter(
  (question: RecruitmentQuestion) => question.is_required,
);
    return (
    <div className="space-y-5">
      ...
      <Section title="Hiring Details">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-1 font-medium">
              {role.hiring.expected_joining_date || "—"}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 font-medium">
              {role.hiring.locations.length
                ? role.hiring.locations.join(", ")
                : "—"}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 font-medium">
              {role.hiring.travel_required ? "Yes" : "No"}
            </div>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <div className="text-xs text-muted-foreground">
              Uses Recruitment Defaults
            </div>

            <div className="mt-1 font-medium">
              {role.eligibility.useRecruitmentDefaults ? "Yes" : "No"}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 font-medium">
              {role.eligibility.willing_to_relocate_required ? "Yes" : "No"}
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="mt-1 whitespace-pre-wrap font-medium">
              {role.eligibility.additional_requirements || "—"}
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section title="Questions">
        <div className="mb-4 flex gap-6 text-sm">
          <span>
            <strong>{role.questions.length}</strong> Total
          </span>
      ...
      <Section title="Recruitment Timeline">
        {role.timeline.length === 0 ? (
          <div className="text-sm text-muted-foreground">
            No recruitment timeline configured.
          </div>
        ) : (
          <div className="space-y-3">
