...
export function SettingsTab({
  ...
  const [allowRestrictedStudents, setAllowRestrictedStudents] =
    useState(false);

  const [allowPlacedStudents, setAllowPlacedStudents] =
    useState(false);

  const [restrictedScope, setRestrictedScope] =
    useState<OverrideScope>("ALL");

  const [placedScope, setPlacedScope] =
    useState<OverrideScope>("ALL");

  const [visibility, setVisibility] = useState(
    summary?.applicationStatus === "Closed"
      ? "HIDDEN"
      : "VISIBLE",
  );
    ...
    <div className="space-y-6">
          ...
          <div className="rounded-xl border p-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() =>
                  setAllowRestrictedStudents(!allowRestrictedStudents)
                }
                className={
                  allowRestrictedStudents
            ...
            {allowRestrictedStudents && (
              <>
                ...
                <ScopeSelector
                  value={restrictedScope}
                  onChange={setRestrictedScope}
                />
          ...
          <div className="rounded-xl border p-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() =>
                  setAllowPlacedStudents(!allowPlacedStudents)
                }
                className={
                  allowPlacedStudents
                    ? "rounded-full bg-green-600 px-5 py-2 text-sm font-medium text-white"
      ...
      >
        ...
        <div className="mt-5 rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
          This preview is temporary. Changes are applied only after saving the
