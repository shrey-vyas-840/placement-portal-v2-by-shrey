...
export function ApplicantsTab({ opportunityId }: ApplicantsTabProps) {
  ...
  const [answers, setAnswers] = useState<RecruitmentQuestionAnswer[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadApplicants() {
      ...
      try {
        const data = await getRecruitmentApplicants(opportunityId!);
  ...
  const filteredApplicants = useMemo(() => {
    ...
    return applicants.filter((applicant) => applicant.fullName.toLowerCase().includes(query));
  }, [applicants, search]);
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">{filteredApplicants.length} applicant(s)</p>
          </div>
          ...
          <table className="w-full">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="px-5 py-3 text-left">Institute</th>
                ...
                filteredApplicants.map((applicant) => (
                  <tr
                    ...
                    onClick={async () => {
                      ...
                      const [answerResult, documentResult] = await Promise.all([
                        getApplicantQuestionAnswers(applicant.applicationId),

                        getApplicantDocuments(applicant.applicationId),
                      ]);
                  ...
                  >
                    <td className="px-5 py-4 font-medium">{applicant.fullName}</td>

                    <td className="px-5 py-4">{applicant.institute}</td>

                    <td className="px-5 py-4">{applicant.branch}</td>

                    <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-2">
                        {applicant.roles.map((role) => (
                          <span key={role} className="rounded-full bg-primary/10 px-3 py-1 text-xs">
                            {role}
                          </span>
                        ))}
                      </div>
                    ...
                    <td className="px-5 py-4">{new Date(applicant.appliedAt).toLocaleString()}</td>
